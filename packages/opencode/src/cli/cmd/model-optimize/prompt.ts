...
export function buildAgentPrompt(config: ModelOptConfig): string {
  ...
  const vars: Record<string, string> = {
    ...
    CONCURRENCY: String(config.concurrency ?? 16),
    INPUT_LEN: String(config.inputLen ?? 1024),
    OUTPUT_LEN: String(config.outputLen ?? 1024),
    NUM_PROMPTS: String(config.numPrompts ?? 100),
  }
...
- **Docker mode** (preferred): If \`env_info.json\` has \`env_type: "docker"\`, prefix all commands with \`docker exec $CONTAINER_NAME bash -c "..."\` and use \`/workspace/output\` as the output directory inside the container. Set \`HIP_VISIBLE_DEVICES=$BEST_GPU\`.
- **venv mode** (fallback): If \`env_type: "venv"\`, activate venv before running Python: \`source ${vars.OUTPUT_DIR}/venv/bin/activate\`
- **NEVER modify system Python packages** in /opt/, /usr/, or site-packages/

## 🎯 CRITICAL PRINCIPLE: Data-Driven Decisions Only
- ✅ **DO**: Read shapes from \`inference_shapes.json\`, analyze \`bottlenecks.json\` and \`analysis_summary.json\`
- ✅ **DO**: Use TraceLens roofline data from \`analysis_summary.json\` and \`unified_perf_summary.csv\` to prioritize which (operator, shape) to optimize
...
2. **If a phase fails, debug and fix before proceeding — do NOT skip**
3. **Integrate optimized kernels via vLLM CustomOp.register_oot() — NEVER modify installed packages**
4. **All optimized kernels go in ${vars.OPTIMIZED_DIR}/ with a vLLM plugin generated by \`generate_vllm_plugin.py\`**

## Optimization Priority (Phase 5-6)
1. **FIRST**: Create and optimize FUSED kernels based on \`analysis_summary.json\` hottest shapes
2. **THEN**: Optimize remaining individual kernels at actual profiled shapes
